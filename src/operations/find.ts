...
export interface FindManyArgs<T, S extends SelectInput<T>> {
  where?: WhereInput<T>;
  select?: S;
...
> {
  ...
  async findMany<S extends SelectInput<InferSchema<TSchema>>>(
    args: FindManyArgs<InferSchema<TSchema>, S> = {}
  ): Promise<
    S extends undefined
      ? Array<InferSchema<TSchema>>
      : Array<SelectResult<InferSchema<TSchema>, S>>
  > {
