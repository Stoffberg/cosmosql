...
> {
	 ...
	 * @param options.enableCrossPartitionQuery - Allow cross-partition deletions
	...
	async deleteMany(options: BulkDeleteOptions<InferSchema<TSchema>>): Promise<BulkDeleteResult> {
		...
		if (!confirm) {
			throw new Error(
				"Must set confirm: true to execute bulk delete. This action cannot be undone.",
			);
		}
			...
			await processBatchesWithConcurrency(batches, maxConcurrency, async (batch, batchIndex) => {
				...
				if (onProgress) {
					...
					const progressStats: BulkProgressStats = {
						total,
						processed,
						updated: result.deleted, // Use 'updated' field to mean 'deleted' for consistency
